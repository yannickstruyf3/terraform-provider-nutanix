...
func resourceNutanixVirtualMachine() *schema.Resource {
	return &schema.Resource{
		...
		StateUpgraders: []schema.StateUpgrader{
			...
			{
				Type:    resourceNutanixVirtualMachineInstanceResourceV1().CoreConfigSchema().ImpliedType(),
				Upgrade: resourceVirtualMachineInstanceStateUpgradeV1,
				Version: 1,
			},
		},
...
func resourceNutanixVirtualMachineRead(d *schema.ResourceData, meta interface{}) error {
	...
	flatDiskList, err := flattenDiskListFilterCloudInit(d, resp.Spec.Resources.DiskList, resp.Spec.Resources.GuestCustomization)
	if err != nil {
...
func resourceVirtualMachineInstanceStateUpgradeV1(is map[string]interface{}, meta interface{}) (map[string]interface{}, error) {
	log.Printf("[YST] Entering resourceVirtualMachineInstanceStateUpgradeV1")
	dummyD := schema.ResourceData{}
	utils.PrintToJSON(is, "[YST] PRE is")
	conn := meta.(*Client).API
	setVMTimeout(meta)
	vmID := is["id"].(string)
	var err error
	// Make request to the API
	resp, err := conn.V3.GetVM(vmID)
	if err != nil {
		return is, fmt.Errorf("error reading Virtual Machine %s: %s", vmID, err)
	}
	fD, err := flattenDiskListFilterCloudInit(&dummyD, resp.Spec.Resources.DiskList, resp.Spec.Resources.GuestCustomization)
	utils.PrintToJSON(fD, "[YST] FD: ")
	if err != nil {
		return is, fmt.Errorf("error migrating")
	}
	is["disk_list"] = fD
	utils.PrintToJSON(is, "[YST] POST is")
	return is, nil
	// return resourceNutanixCategoriesMigrateState(is, meta)
}
...
func resourceNutanixVirtualMachineInstanceResourceV1() *schema.Resource {
	return &schema.Resource{
		Schema: map[string]*schema.Schema{
			"cloud_init_cdrom_uuid": {
				Type:     schema.TypeString,
