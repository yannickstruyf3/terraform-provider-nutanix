...
func resourceNutanixVolumeGroupUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("flash_mode") {
		res.FlashMode = utils.StringPtr(d.Get("flash_mode").(string))
	}

	if d.HasChange("file_system_type") {
		res.FileSystemType = utils.StringPtr(d.Get("file_system_type").(string))
	}

	if d.HasChange("sharing_status") {
		res.SharingStatus = utils.StringPtr(d.Get("sharing_status").(string))
	}
			...
			if len(n) > 0 {
				...
				for k, nc := range n {
					...
					if value, ok := val["iscsi_initiator_name"]; ok && value.(string) != "" {
						attachment.IscsiInitiatorName = utils.StringPtr(value.(string))
					}
	...
	if d.HasChange("iscsi_target_prefix") {
		res.IscsiTargetPrefix = utils.StringPtr(d.Get("iscsi_target_prefix").(string))
	}
	spec.Resources = res
	request.Metadata = metadata
	request.Spec = spec
...
func getVolumeGroupResources(d *schema.ResourceData, vg *v3.VolumeGroupResources) error {
	if v, ok := d.GetOk("flash_mode"); ok {
		vg.FlashMode = utils.StringPtr(v.(string))
	}

	if v, ok := d.GetOk("file_system_type"); ok {
		vg.FileSystemType = utils.StringPtr(v.(string))
	}

	if v, ok := d.GetOk("sharing_status"); ok {
		vg.SharingStatus = utils.StringPtr(v.(string))
	}
		...
		if len(n) > 0 {
			...
			for k, nc := range n {
				...
				if value, ok := val["vmdisk_uuid"]; ok && value.(string) != "" {
					disk.VmdiskUUID = utils.StringPtr(value.(string))
				}

				if value, ok := val["index"]; ok && value.(int) >= 0 {
					disk.Index = utils.Int64Ptr(int64(value.(int)))
				}
				...
				if value, ok := val["disk_size_mib"]; ok && value.(int) >= 0 {
					disk.DiskSizeMib = utils.Int64Ptr(int64(value.(int)))
				}

				if value, ok := val["storage_container_uuid"]; ok && value.(string) != "" {
					disk.StorageContainerUUID = utils.StringPtr(value.(string))
				}
	...
	if v, ok := d.GetOk("iscsi_target_prefix"); ok {
		vg.IscsiTargetPrefix = utils.StringPtr(v.(string))
	}
